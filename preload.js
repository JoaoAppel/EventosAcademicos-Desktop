...
// Be permissive: if a module is not available, don't throw — renderer will check window.XYZ
(() => {
  // minimal safe pathJoin fallback (avoid hard dependency on builtin 'path' if unavailable)
  let pathJoin = (...parts) => parts.filter(Boolean).join('/');
  try {
    const _path = require('path');
    pathJoin = (...parts) => _path.join(...parts);
  } catch (e) {
    // ignore, use fallback
  }

  // Try loading vendor libs but don't fail startup if they're missing
  let idbKeyval = null;
  try { idbKeyval = require('idb-keyval'); } catch (e1) {
    try { idbKeyval = require(pathJoin(__dirname, 'node_modules', 'idb-keyval', 'dist', 'compat.cjs')); } catch (e2) {
      try { idbKeyval = require(pathJoin(__dirname, 'node_modules', 'idb-keyval', 'dist', 'index.cjs')); } catch (e3) { idbKeyval = null; }
    }
  }

  let XLSXlib = null;
  try { XLSXlib = require('xlsx'); } catch (e) { try { XLSXlib = require(pathJoin(__dirname, 'node_modules', 'xlsx')); } catch (e2) { XLSXlib = null; } }

  let jspdfLib = null;
  try { jspdfLib = require('jspdf'); } catch (e) { try { jspdfLib = require(pathJoin(__dirname, 'node_modules', 'jspdf')); } catch (e2) { jspdfLib = null; } }

  let jsQRlib = null;
  try { jsQRlib = require('jsqr'); } catch (e) { try { jsQRlib = require(pathJoin(__dirname, 'node_modules', 'jsqr')); } catch (e2) { jsQRlib = null; } }
  ...
  try {
    ...
    console.warn('preload: could not expose vendor globals:', e && e.message);
  }
})();
